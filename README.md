...
| Тесты | pytest (24 теста), FastAPI TestClient, мок-сервер OpenAI-совместимого API (`scripts/mock_llm_server.py`) |
...
```bash
git clone https://github.com/BAITC-Hacks/hack-8cb43e3f-ai-nam.git
...
Требуется: Python 3.11+, Node.js 20+ (CI использует Node 22). Необязательно: Tesseract OCR с языками `rus` и `kaz`
...
наблюдателя, удаление только своих проектов, обязательную смену временного пароля, разделение входа администратора,
выгрузку Excel на казахском, запись текста документов в Excel без выполнения формул и ИИ-режим на мок-сервере. Те же тесты, сборка интерфейса, сборка
