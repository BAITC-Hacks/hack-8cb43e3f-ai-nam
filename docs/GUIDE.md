...
выгрузку Excel на казахском, запись текста документов в Excel без выполнения формул и ИИ-режим на мок-сервере.
Те же тесты, сборка интерфейса, сборка Docker-образа и тесты второго прототипа AI-NAM запускаются в GitHub Actions
