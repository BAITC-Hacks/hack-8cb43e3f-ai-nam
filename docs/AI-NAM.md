...
> Второй прототип команды. Файлы приложения: `app/`, `web/`, `run.py`, `tests/`, `deploy/`, `scripts/backup.py`, `start.ps1`, `requirements.txt`, `requirements.lock.txt`. Команды ниже выполняются из корня репозитория. У AI-NAM своё окружение `.venv-ainam`: `.venv` занято зависимостями OrgStruct AI. Основное приложение репозитория описано в [README](../README.md).
...
.\.venv-ainam\Scripts\python.exe -m pip install -r requirements.lock.txt
.\.venv-ainam\Scripts\python.exe run.py
```
...
Linux: `python3 -m venv .venv-ainam`, затем `.venv-ainam/bin/python -m pip install -r requirements.lock.txt` и `.venv-ainam/bin/python run.py`.
...
```powershell
.\.venv-ainam\Scripts\python.exe scripts/backup.py --output D:/AI-NAM-backups
```
...
```powershell
.\.venv-ainam\Scripts\python.exe -m unittest discover -v
```
